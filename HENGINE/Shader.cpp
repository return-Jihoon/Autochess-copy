...
unsigned int Shader::SizeOfFormat(DXGI_FORMAT _Fmt) 
{
	switch (_Fmt)
	{
...
char Shader::VERBUFFERTEXT[256];
// std::map<unsigned int, HPTR<H3DBUFFER>> HSHADER::m_BufferPool[HSHADER::SHADERTYPE::SHT_END];

Shader::Shader(SHADERTYPE _Type) : m_pBlob(nullptr), m_pErrBlob(nullptr), m_Type(_Type)
{
}
...
void Shader::CreateCB(const wchar_t* _Name, unsigned int _Byte, UINT _Reg, CBTYPE _Type /*= CBTYPE::NONE*/)
{
...
void Shader::CreateSMPDATA(const wchar_t* _Name, UINT _Reg)
{
...
void Shader::CreateTEXDATA(const wchar_t* _Name, UINT _Reg)
{
...
bool Shader::IsOver(Shader* _pShader) 
{
	if (_pShader == this)
	{
		return false;
