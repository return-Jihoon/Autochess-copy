...
{
	m_Original = true;
	m_AllShader.resize(Shader::SHADERTYPE::SHT_END);
}
...
void RenderPath::Init(Renderer* _Parent) 
{
	...
	for (size_t i = 0; i < m_AllShader.size(); i++)
	{
		...
		if (0 != m_CBInfoSheet.size())
		{
			...
			{
				if (CBInfoStart->second->m_Data.m_CBType == Shader::CBTYPE::TRANS)
				{
...
{
	m_BLEND = Resources::Find<Blend>(_Name);
...
{
	m_DSSTATE = Resources::Find<DepthStencilState>(_Name);
...
{
	m_RSSTATE = Resources::Find<RasterizerState>(_Name);
...
void RenderPath::ShaderSetting(Shader* _Shader)
{
	{
		...
		for (; Start != End; ++Start)
		{
			...
			NewCBSET->m_H3DBUFFER = new D3DBuffer();
...
{
	HPTR<VertexShader> SHADER = Resources::Find<VertexShader>(_Name);
	...
	if (nullptr != m_AllShader[Shader::SHADERTYPE::SHT_VS])
	{
		BOOM;
	}

	for (size_t i = 0; i < Shader::SHADERTYPE::SHT_END; i++)
	{
		if (nullptr == m_AllShader[i])
		{
			continue;
	...
	ShaderSetting(SHADER.Convert<Shader>());
	m_AllShader[Shader::SHADERTYPE::SHT_VS] = SHADER;
}

void RenderPath::SetPxlShader(const wchar_t* _Name) 
{
	HPTR<PixelShader> SHADER = Resources::Find<PixelShader>(_Name);
	...
	if (nullptr != m_AllShader[Shader::SHADERTYPE::SHT_PS])
	{
		BOOM;
	}
	...
	ShaderSetting(SHADER.Convert<Shader>());
	m_AllShader[Shader::SHADERTYPE::SHT_PS] = SHADER;
}
...
void RenderPath::LinkCB(const wchar_t* _Name, void* _LinkData) {
	...
	CBInfoStart->second->m_Data.m_CBType = Shader::CBTYPE::LINK;
...
void RenderPath::LinkSMP(const wchar_t* _Name, const wchar_t* _SmpName)
{
	...
	SmpInfoStart->second->m_Smp = Resources::Find<SamplerState>(_SmpName);
