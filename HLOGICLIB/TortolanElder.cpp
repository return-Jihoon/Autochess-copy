...
void TortolanElder::Skill()
{
	...
	if (isSkillStart == false)
	{
		...
		HPTR<SpriteRender> chargeRender = chargeActor->AddComponent<SpriteRender>(RENDERGROUP::RG_PLAYER);
	...
	if (pFbxAnimation->GetCurClip()->m_AniEnd == true)
	{
		if (pAttTarget != nullptr)
		{
			...
			HPTR<SpriteRender> skillRender = chessPlayer2->AddComponent<SpriteRender>(RENDERGROUP::RG_PLAYER);
