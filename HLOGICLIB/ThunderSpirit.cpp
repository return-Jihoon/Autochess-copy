...
void ThunderSpirit::MakeProjecTile()
{
	if (pAttTarget != nullptr)
	{
		...
		HPTR<SpriteRender> battleTopFrameSR = PLAYER2->AddComponent<SpriteRender>(RG_PLAYER);
...
void ThunderSpirit::Skill()
{
	...
	if (skillOn == false)
	{
		...
		HPTR<SpriteRender> skillRender = newActor->AddComponent<SpriteRender>(RENDERGROUP::RG_PLAYER);
